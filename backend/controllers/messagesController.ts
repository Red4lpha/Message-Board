...
const messages_create = async (req: Request, res: Response) => {
  ...
  return Users.findOne({ _id: user_id })
    ...
    .catch((error) => {
      logging.error(
        'messages_create',
        'Error cannot create new message',
        error
      );
      return res.status(500).json({ error });
    });
  ...
  return Messages.findOne({ _id: parentID })
    .then(async (parentMessage) => {
      if (!parentMessage) {
        return res
          .status(400)
          .json({ Message: 'Cannot find original message' });
      } else if (parentMessage.deleted) {
        return res
          .status(401)
          .json({ Message: 'Cannot reply on a deleted message' });
      } else {
        //? grabs the parent messages ancestors
        const ancestors: MessagesInterface['ancestors'] =
          parentMessage.ancestors;
    ...
    .then(async (message) => {
      if (message?.owner.name_id) {
        if (user_id?.toString() !== message.owner.name_id.toString()) {
          return res
            .status(401)
            .json({ message: 'Unauthorized user - not the owner' });
        }
        message.text = text;
        await message.save();
  ...
  return Messages.findOne({ _id: message_id })
    .then(async (message) => {
      if (user_id == message?.owner.name_id) {
        return res
          .status(400)
          .json({ message: 'Cannot vote on your own message' });
      }
      ...
      if (tempHolder !== null) {
        if (tempHolder?.includes(user_id)) {
          return res
            .status(400)
            .json({ message: 'Already voted on this message' });
        }
      }
      ...
      if (vote != 1 && vote != -1) {
        ...
        logging.info(
          'messages_vote',
          `User: ${user_id} voted: ${vote} on ${message_id}`
        );
        return res.status(201).json(message);
      } else {
