...
>('messages/createMessage', async (messageText, thunkAPI: any) => {
  try {
...
export const replyMessage = createAsyncThunk<
  { rejectValue: ValidationErrors },
  messagesDataInterface
>('messages/replyMessage', async (messageData, thunkAPI: any) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const updateMessage = createAsyncThunk<
  { rejectValue: ValidationErrors },
  messagesDataInterface
>('messages/updateMessage', async (messageData, thunkAPI: any) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
//? Vote on a message
export const voteMessage = createAsyncThunk<
  { rejectValue: ValidationErrors },
  messagesDataInterface
>('messages/voteMessage', async (messageData, thunkAPI: any) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const deleteMessage = createAsyncThunk<
  { rejectValue: ValidationErrors },
  messagesDataInterface
>('messages/deleteMessage', async (messageData, thunkAPI: any) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateMessage.fulfilled, (state, action: PayloadAction<any>) => {
        ...
        index !== -1
          ? (state.messagesArray[index].text = text)
          : (state.isError = true);
      })
      ...
      .addCase(voteMessage.fulfilled, (state, action: PayloadAction<any>) => {
        ...
        let index = state.messagesArray.findIndex((msg) => msg._id === _id);
        index !== -1
          ? (state.messagesArray[index].votes = votes)
          : (state.isError = true);
      })
      ...
      .addCase(deleteMessage.fulfilled, (state, action: PayloadAction<any>) => {
        ...
        let index = state.messagesArray.findIndex((msg) => msg._id === _id);
        deleted ? state.messagesArray.splice(index, 1) : (state.isError = true);
      })
