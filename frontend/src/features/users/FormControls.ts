...
export const FormControls = () => {
  ...
  const PostContactForm = async (values: any) => {
    if (form === 'login') await dispatch(login(values));
    else if (form === 'register') await dispatch(register(values));
  ...
  const validate: any = (fieldValues = values) => {
    ...
    if ('password2' in fieldValues) {
      ...
      if (fieldValues.password2) {
        if (fieldValues.password2 && fieldValues.password2 !== values.password)
